...
const EarTrainer: React.FC = () => {
  ...
  const { isHandfree, setIsHandfree, mode } = globalSettings;

  const currentGameSettings =
    mode === 'free' ? FreeTrainerSettings : ChallengeTrainerSettings;
  const {
    playNote,
    rootNote,
    setActiveNote,
  ...
  const components = [
    {
      id: 'game',
      label: 'settings.GameSettings',
      component: GameSettings,
      props: { currentGameSettings: currentGameSettings },
    },
    {
      id: 'practice',
    ...
    const handleKeyPress = (event: KeyboardEvent) => {
      ...
      if (degreeIndex !== undefined) {
        ...
        const button2 = document.querySelector(
          `button[data-note="${enharmonicNote}"]`
        );
        if (div1) {
    ...
    <div className="relative">
        ...
        <HeaderButtons title="pocket mode!">
          {gameState != 'end' && (
            <HeaderButton
              ...
              title={
                isHandfree ? t('buttons.handfreeOff') : t('buttons.handfreeOn')
              }
              className={
                isHandfree ? 'bg-bg-common' : 'bg-text-main text-text-main'
              }
            >
              <span>
      ...
      <div className="h-[calc(100svh-64px)] bg-bg-main">
        <div className="max-w-2xl mx-auto h-full flex flex-col justify-between py-4 px-6">
          <div className="pr-4">
            {' '}
            {/* Add right padding for scrollbar placeholder */}
            <SettingsPanel
          ...
          {mode === 'free' ? (
            <FreeMode FreeTrainerSettings={FreeTrainerSettings} />
          ) : (
            <ChallengeMode

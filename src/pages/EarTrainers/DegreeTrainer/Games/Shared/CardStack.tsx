    ...
    return (
      ...
      ${isCorrectAnswer && isAdvance !== 'No' ? 'bg-green-500 text-white' : 'bg-bg-accent text-text-primary'}
                  ${isDisabled ? 'opacity-50 pointer-events-none' : ''}`}
                        ...
                        >
                            <span className="select-none">{useSolfege ? SolfegeMapping[note.name as keyof typeof SolfegeMapping] || note.name : note.name}</span>
                ...
                {filteredNotes.map((note, index) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            >
                                {useSolfege ? SolfegeMapping[note.name as keyof typeof SolfegeMapping] || note.name : note.name}
                            </Button>
                        </motion.div>
                    );
                })}

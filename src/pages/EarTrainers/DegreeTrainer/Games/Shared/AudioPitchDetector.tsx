...
const AudioPitchDetector = ({ rootNote, setActiveNote, useSolfege, currentNote }: AudioPitchDetectorProps) => {
    ...
    const handleGame = (pitchHz: number, currentNote: string) => {
        ...
        if (!noteName) return;
        const degree = calculateDegree(noteName, rootNote) as keyof typeof DegreeToDistance | keyof typeof SolfegeMapping | string;
        ...
        <div className="flex flex-col gap-4 items-center">
            <div className="text-2xl font-bold text-text-primary">
                {detectedNote === "?"
                    ? "?"
                    : useSolfege
                        ? (SolfegeMapping[calculateDegree(detectedNote, rootNote) as keyof typeof SolfegeMapping] ?? "?")

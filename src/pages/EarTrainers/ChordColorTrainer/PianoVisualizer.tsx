...
const getBlackKeyX = (midiNote: number, startNote: number): number => {
  ...
  return (
    SVG_PADDING_X +
    precedingWhiteKeyIndex * WHITE_KEY_WIDTH -
    BLACK_KEY_WIDTH / 2
  );
};
...
const PianoVisualizer: React.FC<PianoVisualizerProps> = ({
  ...
  const setPlayMidiSounds = useSoundSettingsStore(
      ...
      <div className="flex  justify-between flex-col">
        ...
        <div className="text-md min-h-6 min-w-[200]  flex">
          {detectedChords.length > 1 &&
            detectedChords
              .slice(1)
              .map((chord, i) => <div key={i}>{chord + ','}&nbsp;</div>)}
        </div>
      </div>
      ...
      >
        ...
        >
          ...
          {playMidiSounds ? (
            <>
              <path
                d="M242 24 Q250 30 242 36"
                stroke="black"
                strokeWidth="3"
                fill="none"
              />
              <path
                d="M248 20 Q258 30 248 40"
                stroke="black"
                strokeWidth="3"
                fill="none"
              />

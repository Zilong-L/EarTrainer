...
// Use any for the full library object to avoid overly strict index signatures
const SampleLibrary: any = {
  minify: false,
  ext: '.[mp3|ogg]',
  baseUrl: '/samples/',
  list: [
  ...
  synthList: ['triangle', 'square', 'sawtooth', 'pad'],
  onload: null,
  ...
  load(arg: LoadOptions) {
    ...
    // synth branch
    if (typeof t.instruments === 'string' && this.synthList.includes(t.instruments)) {
      return this.loadSynth(t.instruments);
    }

    // Update extensions if arg given
    if (t.ext && t.ext !== this.ext) {
      this.setExt(t.ext);
      t.ext = this.ext;
    }

    const getMinBy = (quality?: Quality) => {
      switch (quality) {
        case 'low':
          return 6;
        case 'medium':
    ...
    return buildSampler(t.instruments);
  },

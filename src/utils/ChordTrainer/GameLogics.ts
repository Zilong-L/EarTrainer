...
const compareChords = (detectedChords: string[], targetChord: string, ignoreTranspose = false): boolean => {
...
function getInversion(root: string, symbol: string, position = -1): string | string[] {
...
const sortChordsByCommonality = (chordResultObjects: ChordObject[]): string[] => {
  const sortedChordResultObjects = chordResultObjects.sort((a, b) => {
    const aVal = chordCommonnessHash[a.type as keyof typeof chordCommonnessHash] ?? Number.MAX_SAFE_INTEGER;
    const bVal = chordCommonnessHash[b.type as keyof typeof chordCommonnessHash] ?? Number.MAX_SAFE_INTEGER;
    return aVal - bVal;
  });
  return sortedChordResultObjects.map(chord => (chord as any).symbol ?? `${chord.tonic}${chord.type}`);
};
...
const getChords = (notes: number[]): string[] | undefined => {
  ...
  let chordResultObjects = chordResult.map(chord => Chord.get(chord)) as ChordObject[];

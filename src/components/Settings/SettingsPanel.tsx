...
interface SettingsPanelProps {
    isOpen: boolean;
    onClose?: () => void;
    components: SettingsNavItem[];
    title?: string; // i18n key
}

function SettingsPanel({ isOpen, onClose, components, title = 'settings.Settings' }: SettingsPanelProps) {
    ...
    const getNavButtonClass = (pageId: string) => {
            ...
            <div className="relative w-[90%] max-w-4xl h-[80vh] bg-bg-common rounded-lg shadow-xl overflow-hidden">
                ...
                <div className="flex items-center p-4 border-b border-bg-accent">
                    ...
                    <div className="w-8 md:hidden">
                        {currentPage !== 'home' && (
                            <button onClick={() => setCurrentPage('home')} className="p-2 hover:bg-bg-hover rounded-full">
                    ...
                    <h2 className="text-xl font-semibold text-text-primary flex-1 text-center md:text-left">{t(title)}</h2>

                    <button onClick={handleClose} className="p-2 hover:bg-bg-hover rounded-full">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-text-secondary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="flex h-[calc(100%-4rem)]">
                    ...
                    <div className="w-48 border-r border-bg-accent bg-bg-common h-full hidden md:block">
                        <nav className="p-4 space-y-2 bg-bg-common">
                            {components.map((page) => (
                                <button key={page.id} onClick={() => setCurrentPage(page.id)} className={`w-full px-4 py-3 ${getNavButtonClass(page.id)}`}>
                                    {t(page.label)}
                                </button>
                            ))}
                        </nav>
                    ...
                    <div className="flex-1 flex flex-col pb-6 bg-bg-common">
                        ...
                        >
                                ...
                                <div className="p-6">
                                    ...
                                    <div className="md:hidden">
                                        <div className="grid grid-cols-1 gap-4">
                                            {components.map((page) => (
                                                <button key={page.id} onClick={() => setCurrentPage(page.id)} className={`p-6 ${getNavButtonClass(page.id)}`}>
                                                    <h3 className="text-lg font-semibold">{t(page.label)}</h3>
                                                </button>
                                            ))}
                                        </div>
                                    </div>
                            ...
                            {currentPage === 'home' && <div className="hidden md:block text-center text-text-secondary">
